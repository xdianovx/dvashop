...
test('failed cancellation rolls back without partially releasing redemption', function (): void {
    $actor = User::factory()->admin()->create();
    $promo = PromoCode::factory()->create(['usage_limit' => 1]);
    $order = Order::factory()->create(['promo_code_id' => $promo->getKey()]);
    $redemption = PromoCodeRedemption::factory()->for($promo)->for($order)->create();
    $inventory = Mockery::mock(OrderInventoryService::class);
    $inventory->shouldReceive('restoreForCancellation')->once()->andThrow(new RuntimeException('restore failed'));
    $service = new OrderOperationsService($inventory);

    expect(fn () => $service->update($actor, $order, ['status' => OrderStatus::Canceled]))
        ->toThrow(RuntimeException::class, 'restore failed');
