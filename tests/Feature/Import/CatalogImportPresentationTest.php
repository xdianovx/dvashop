...
test('Validation page presents Filament upload help and compact log controls', function () {
    $this->actingAs(User::factory()->admin()->create());

    $this->get('/admin/imports/catalog')
        ->assertOk()
        ->assertSee('Загрузка файла')
        ->assertSee('Используется для безопасного обновления и архивирования только товаров этого источника.')
        ->assertSee('Выбрать CSV/XLSX')
