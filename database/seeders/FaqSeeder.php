...
class FaqSeeder extends Seeder
{
    ...
    private function items(): array
    {
        return [
            ...
            'payment_methods' => ['category' => 'payment_delivery', 'question' => 'Какие способы оплаты доступны?', 'answer' => 'После подтверждения заказа можно оплатить картой или через СБП по ссылке либо QR-коду, по счёту для юридических лиц, а при получении — только если этот способ активен при оформлении.', 'is_featured' => false, 'is_active' => true, 'position' => 10],
            'delivery_process' => ['category' => 'payment_delivery', 'question' => 'Как осуществляется доставка?', 'answer' => 'Отправляем транспортными компаниями по всей России. После оформления менеджер подтвердит способ, стоимость и срок доставки.', 'is_featured' => false, 'is_active' => true, 'position' => 20],
            'delivery_cost' => ['category' => 'payment_delivery', 'question' => 'Сколько стоит доставка?', 'answer' => 'Самовывоз бесплатный. Фиксированная стоимость указывается при оформлении, а стоимость доставки по запросу уточнит менеджер после подтверждения заказа.', 'is_featured' => false, 'is_active' => true, 'position' => 30],
