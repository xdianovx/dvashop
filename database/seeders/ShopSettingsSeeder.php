...
class ShopSettingsSeeder extends Seeder
{
    ...
    private const NAVIGATION = [
        ...
        'how' => ['zone' => NavigationZone::HeaderTop, 'title' => 'Как мы работаем', 'route_name' => 'how', 'position' => 20],
        ...
        'payment' => ['zone' => NavigationZone::FooterDocuments, 'title' => 'Оплата и доставка', 'route_name' => 'payment', 'position' => 10],
    ];

    public function run(): void
    {
        DB::transaction(function (): void {
            $settings = ShopSetting::query()->firstOrNew(['singleton_key' => ShopSetting::SINGLETON_KEY]);
            $defaults = ShopSettingsService::defaults();
            unset($defaults['singleton_key']);
            $this->fillMissing($settings, $defaults)->save();
            ...
            foreach (self::NAVIGATION as $code => $definition) {
                ...
                $item = SiteNavigationItem::query()->firstOrNew(['code' => $code]);

                $this->fillMissing($item, [
