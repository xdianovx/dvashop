        ...
        return $schema
            ...
            ->components([
                ...
                Section::make('Контакты')
                    ->schema([
                        TextInput::make('phone_display')->label('Отображаемый телефон')->placeholder('Не заполнено')->maxLength(100),
                        TextInput::make('phone_href')->label('Телефон для tel:')->placeholder('Не заполнено')->maxLength(32),
                ...
                Section::make('Реквизиты')
                    ->schema([
                        TextInput::make('legal_name')->label('Юридическое название')->placeholder('Не заполнено')->maxLength(255),
                        TextInput::make('inn')->label('ИНН')->placeholder('Не заполнено')->maxLength(12),
                        TextInput::make('ogrn')->label('ОГРН')->placeholder('Не заполнено')->maxLength(15),
                        Textarea::make('legal_address')->label('Юридический адрес')->placeholder('Не заполнено')->rows(3)->maxLength(2000)->columnSpanFull(),
                    ])->columns(2),
                ...
                Section::make('Подвал')
                    ->schema([
                        Textarea::make('footer_copyright')->label('Copyright')->placeholder('Не заполнено')->rows(3)->maxLength(2000),
                        Textarea::make('footer_disclaimer')->label('Дисклеймер')->placeholder('Не заполнено')->rows(3)->maxLength(500),
                    ])->columns(2),
            ]);
    }
